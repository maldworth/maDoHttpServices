...
public class EditTask extends BasicToodledoRequestModel
{
	...
	private long _goalId = -1;
	@SuppressWarnings("unused")
	private int _isTimer = -1;//TODO
	@SuppressWarnings("unused")
	private int _timerVal = -1;//TODO
	private long _parent = -1;
	private int _isCompleted = -1;
	private long _completedOn = -1L;
	private int _reschedule = -1;
	private long _dueDate = -1L;
	private String _dueDateString;
	private DueModifier _dueDateModifier;
	private long _startDate = -1L;
	private long _dueTime = -1L;
	...
	public boolean isReschedule() {
		return _reschedule == 1;
	}

	public void setCompleted(boolean isCompleted) {
		_isCompleted = (isCompleted)?1:0;
	}
	...
	public List<NameValuePair> getQueryParams()	{		
		...
		if(_dueTime == 0L)
			this.addParam("duetime", "0");
		else if(_dueTime > 0L)
			this.addParam("duetime", DateTimeHelper.tohmmaa(DateTimeHelper.fromUnixTime(_dueTime)));
		
		if(_startDate == 0L)
			this.addParam("startdate", "0000-00-00");
		else if(_startDate > 0L)
			this.addParam("startdate", _startDate);//Accepts unix timestamp
		
		if(_startTime == 0L)
			this.addParam("starttime", "0");
